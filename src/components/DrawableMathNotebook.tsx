...
const DrawableMathNotebook: React.FC<DrawableMathNotebookProps> = ({ question, onRecognize }) => {
  ...
  useEffect(() => {
    ...
    if (gridVisible) {
      ...
      for (let x = 0; x <= w; x += size) { gctx.moveTo(x, 0); gctx.lineTo(x, h); }
      for (let y = 0; y <= h; y += size) { gctx.moveTo(0, y); gctx.lineTo(w, y); }
      gctx.stroke();
    } else {
      gctx.clearRect(0, 0, w, h);
    }
  ...
  const handleScan = async () => {
    ...
    try {
      if (isKeyboard) {
        ...
        const { data: { text } } = await Tesseract.recognize(dataUrl, 'eng+heb', opts);
    ...
    <div className="drawable-notebook-container">
      ...
      {isKeyboard ? (
        <div className="keyboard-mode">
          ...
          <ActionButtons scanning={scanning} onScan={handleScan} onClear={handleClear} />
        </div>
      ) : (
        <>
          ...
          <div className="controls-row">
            ...
            <label>ריבועים: {gridCount}×{gridCount}</label>
            <input type="number" min={5} max={50} value={gridCount} onChange={e => setGridCount(+e.target.value)} />
