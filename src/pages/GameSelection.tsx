...
//         <h1
//           className="text-4xl font-bold text-blue-900 mb-4 drop-shadow-lg"
//           dir="rtl"
//         >
//           🎮 Math Games Hub 🎮
//         </h1>
//         <p
//           className="text-lg text-blue-800 max-w-2xl mx-auto leading-relaxed"
//           dir="rtl"
//         >
...
//           💡 עצה: התחילו ממשחק קל והתקדמו לרמות קשות יותר!
//         </p>
//       </div>
//     </div>
//   );
...
import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import { Card, CardContent } from "../components/ui/card"; // Assuming these are still used for basic structure
import { Button } from "../components/ui/button"; // Assuming this is still used for basic structure
import { Play, Lock, Star, Zap, Trophy, Clock } from "lucide-react";
...
export default function GameSelection() {
  ...
  const games = [
    {
      ...
      available: true,
      features: ["חיבור, חיסור, כפל", "3 רמות קושי", "לוח תוצאות", "אפקטי קול"],
      color: "green-yellow", // Use a simplified name for gradient classes
    },
    {
      id: "geometry-quest",
      title: "Geometry Quest",
      ...
      available: false,
      features: ["זיהוי צורות", "חישוב שטחים", "זוויות ומשולשים", "תלת מימד"],
      color: "blue-indigo", // Use a simplified name for gradient classes
    },
  ];

  // Helper function to get the correct gradient class based on game.color
  const getGradientClass = (colorName: string) => {
    switch (colorName) {
      case "green-yellow":
        return "gradient-green-yellow";
      case "blue-indigo":
        return "gradient-blue-indigo";
      default:
        return "";
    }
  };
  ...
  return (
    <div className="game-selection-container">
      ...
      {/* Floating math symbols (commented out as per original) */}
      {/* <div className="floating-symbols">
        <div className="math-symbol math-symbol-plus">+</div>
        <div className="math-symbol math-symbol-times">×</div>
        <div className="math-symbol math-symbol-equals">=</div>
        <div className="math-symbol math-symbol-divide">÷</div>
        <div className="math-symbol math-symbol-sqrt">√</div>
      </div> */}

      <div className="header-section">
        <h1 className="header-title">🎮 Math Games Hub 🎮</h1>
        <p className="header-description">
          ברוכים הבאים למרכז המשחקים המתמטיים! בחרו את המשחק שלכם והתחילו להתאמן
        </p>
        <div className="header-stars">
          <Star className="star-icon" />
          <span>משחקים חינוכיים ומהנים</span>
          <Star className="star-icon" />
        </div>
      </div>
      ...
      <div className="game-cards-grid">
        {games.map((game) => (
          ...
          >
            <div
              className={`game-card-gradient-overlay ${getGradientClass(
                game.color
              )}`}
            ></div>

            {!game.available && (
              <div className="game-card-locked-badge">
                <div>
            ...
            <CardContent className="game-card-content">
              ...
              <div className="game-details-section">
                <div className="game-detail-item">
                  <div className="icon-and-label">
                    <Zap className="detail-icon zap-icon" />
                ...
                <div className="game-detail-item">
                  <div className="icon-and-label">
                    ...
                    <span className="detail-label">זמן משחק:</span>
                  </div>
                  <span className="detail-value">{game.timeLimit}</span>
                </div>
              </div>
              ...
              <Button
                onClick={() => game.available && handleGameSelect(game.id)}
                disabled={!game.available}
                // Dynamically apply a gradient class for the button as well
                className={`play-button ${
                  game.available
                    ? `available ${getGradientClass(game.color)}`
                    : "not-available"
                }`}
              >
                {game.available ? (
              ...
              {hoveredGame === game.id && game.available && (
                <div
                  className={`game-card-hover-pulse-overlay ${getGradientClass(
                    game.color
                  )}-pulse`}
                ></div>
              )}
      ...
      <div className="footer-hint">
        <p>💡 עצה: התחילו ממשחק קל והתקדמו לרמות קשות יותר!</p>
      </div>
    </div>
  );
}
